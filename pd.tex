...
実験手法を工夫する}ことを考える。また、\ul{(2)加速器実験のデータを元に
理論的な考察から新物理の模型を制限する}ことも、電弱相互作用の理解を深
める上で有用である。次の項目で、(1)(2)それぞれのアプローチについてより
具体的に説明する。\rem{ここもっと抽象的に大きく書くべきでは？}
...
■\pmb{\ul{【A】100TeV 粒子加速器を用いた新物理の探索}}（解決方策の(1)に対応）
...
で明らかになるかを調べることである。特に、新物理が特徴的な信号を与える
過程に着目し、背景事象の数自体を減らす、あるいは適切な統計処理により信
号の情報を抜き出せるようにすることを考える。そのために 用いた手法には、
\ul{消失飛跡を用いた長寿命粒子の探索}、および\ul{新物理の信号の関数形
を用いた背景事象との識別}が挙げられる。「これまでの研究結果」の節で、
それぞれについて具体的に述べる。
...
■\pmb{\ul{【A】消失飛跡を用いた長寿命粒子の探索}}（解決方策の(1)に対
応）
